	...
	CNtlVector* pNewHeadingVector, CNtlVector* pDestination)
{
	...
	*pNewHeadingVector = RotateVector(pNewHeadingVector, -fRadian);
	...
	pDestination->x = fCurrentPositionX + fTempX * (float)sin(fRadian) - fTempZ * (1 - cos(fRadian));
	pDestination->y = fCurrentPositionY;
	pDestination->z = fCurrentPositionZ + fTempZ * (float)sin(fRadian) - fTempX * (1 - cos(fRadian));
}
