...
bool SkillTable::SetTableData(void* pvTable, WCHAR* pwszSheetName, std::wstring* pstrDataName, BSTR bstrData)
{
	if (0 == wcscmp(pwszSheetName, L"Table_Data_KOR"))
	{
		...
		else if (0 == wcscmp(pstrDataName->c_str(), L"Skill_Class"))
		{
			...
			if (SKILL_CLASS_FIRST > pSkill->bySkill_Class || SKILL_CLASS_LAST < pSkill->bySkill_Class)
			{
				_ASSERT(0);
				return false;
			}
		...
		else if (0 == wcscmp(pstrDataName->c_str(), L"Skill_Type"))
		{
			...
			if (SKILL_TYPE_FIRST > pSkill->bySkill_Type || SKILL_TYPE_LAST < pSkill->bySkill_Type)
			{
				_ASSERT(0);
				return false;
			}
