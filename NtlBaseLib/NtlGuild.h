...
{
	GUILDID			guildId;
	WCHAR			wszName[MAX_SIZE_GUILD_NAME_IN_UNICODE + 1];
	...
	: public sDBO_GUILD_DATA
{
	...
	WCHAR			wszMasterName[MAX_SIZE_CHAR_NAME_UNICODE + 1];
	DWORD			dwZenny;	
};
...
struct sDBO_GUILD_MEMBER_INFO
{
	...
	{
		::CopyMemory(wszMemberName, pGuildMemberData->wszMemberName, MAX_SIZE_CHAR_NAME_UNICODE * sizeof(WCHAR));
		wszMemberName[MAX_SIZE_CHAR_NAME_UNICODE] = L'\0';
	};
	...
	: public sDBO_GUILD_MEMBER_DATA
{
	SERVERFARMID serverfarmID;
	GUILDID guildID;
	WCHAR wszAccountName[MAX_SIZE_USERID_UNICODE + 1];
};
